...
import os
import asyncio
from .game_stat_extractor_module import preprocess_roi, extract_stats_from_image
...
async def extract_all_stats_async(main_image, model, reader):
    """
    ...
    ai_task = extract_stats_from_image(main_image, model, reader)
    template_result, ai_result = await asyncio.gather(template_task, ai_task)
    # Compose results
    result = {}
    result.update(template_result)
    if 'meso' in ai_result:
        result['meso'] = ai_result['meso']
    return result 